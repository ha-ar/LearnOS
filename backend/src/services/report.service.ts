...
export class ReportService {
   ...
   */
  static mapMasteryToPlainLanguage(masteryLevel: string, scoreRatio: number = 0.5): { label: string; percent: number } {
   ...
   */
  static async generateWeeklyReport(learnerId: string, weekLabel: string = '2026-W30'): Promise<WeeklyReportRow> {
    ...
    const compRes = await query(
      `SELECT c.topic, lcs.mastery_level, lcs.mastery_score, lcs.learner_confidence
    ...
    const glance: WeeklyReportGlance = {
    ...
    const confidenceSummary: WeeklyReportConfidence = {
      average_score: 3.2,
      scale: 5,
    ...
    const upsertRes = await query(
      `INSERT INTO weekly_reports (
       ...
       RETURNING *`,
      [learnerId, parentId, tenantId, weekLabel, weekStart, weekEnd, JSON.stringify(reportData)]
    );
   ...
   */
  static async getLatestWeeklyReport(learnerId: string): Promise<WeeklyReportRow> {
    const res = await query(
      `SELECT * FROM weekly_reports WHERE learner_id = $1 ORDER BY week_start DESC LIMIT 1`,
      [learnerId]
    );
   ...
   */
  static async getWeeklyReport(learnerId: string, weekLabel: string): Promise<WeeklyReportRow | null> {
    const res = await query(
      `SELECT * FROM weekly_reports WHERE learner_id = $1 AND week_label = $2`,
      [learnerId, weekLabel]
    );
   ...
   */
  static async listWeeklyReports(learnerId: string): Promise<WeeklyReportRow[]> {
    const res = await query(
      `SELECT * FROM weekly_reports WHERE learner_id = $1 ORDER BY week_start DESC`,
      [learnerId]
    );
   ...
   */
  static async sendWeeklyReportEmail(learnerId: string, weekLabel: string = '2026-W30'): Promise<{ sent: boolean; recipient: string; html_body: string }> {
    ...
    const reportData: WeeklyReportData = typeof reportRow.report_data === 'string' ? JSON.parse(reportRow.report_data) : reportRow.report_data;
   ...
   */
  static renderReportHtml(report: WeeklyReportData): string {
