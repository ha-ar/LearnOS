...
export class AdminService {
  ...
  static async logAction(params: {
    ...
    await query(
      `INSERT INTO audit_log (actor_id, actor_role, tenant_id, action, resource_type, resource_id, payload_summary, result, reason, created_at)
  ...
  static async getUserById(tenantId: string, userId: string) {
    const res = await query(
      `SELECT u.id, u.email, u.name, u.role, u.tenant_id, u.is_active, u.created_at, u.updated_at,
  ...
  static async updateUser(tenantId: string, userId: string, params: { name?: string; role?: string; grade?: string; is_active?: boolean }, adminId: string) {
    ...
    if (params.name || params.role !== undefined || params.is_active !== undefined) {
      await query(
        ...
        [params.name || null, params.role || null, params.is_active !== undefined ? params.is_active : null, userId, tenantId]
      );
    }
   ...
   */
  static async deactivateUser(tenantId: string, userId: string, adminId: string) {
    const res = await query(
      `UPDATE users SET is_active = false, updated_at = NOW() WHERE id = $1 AND tenant_id = $2 RETURNING id, email, is_active`,
      [userId, tenantId]
    );
    if (res.rows.length === 0) {
      throw { status: 404, error: 'User not found', code: 'NOT_FOUND' };
   ...
   */
  static async assignMentor(tenantId: string, learnerId: string, mentorId: string, adminId: string) {
    // Verify learner
    const learnerCheck = await query(`SELECT role FROM users WHERE id = $1 AND tenant_id = $2`, [learnerId, tenantId]);
    ...
    // Verify mentor
    const mentorCheck = await query(`SELECT role FROM users WHERE id = $1 AND tenant_id = $2`, [mentorId, tenantId]);
    ...
    await query(`UPDATE learner_mentor_assignments SET is_active = false WHERE learner_id = $1`, [learnerId]);

    // Insert new active assignment
    const res = await query(
      `INSERT INTO learner_mentor_assignments (learner_id, mentor_id, tenant_id, is_active, assigned_at, assigned_by)
       VALUES ($1, $2, $3, true, NOW(), $4)
       ON CONFLICT (learner_id, mentor_id) DO UPDATE SET is_active = true, assigned_at = NOW(), assigned_by = EXCLUDED.assigned_by
       RETURNING *`,
      [learnerId, mentorId, tenantId, adminId]
    );
   ...
   */
  static async assignParent(tenantId: string, learnerId: string, parentId: string, consentGiven: boolean, adminId: string) {
    const learnerCheck = await query(`SELECT role FROM users WHERE id = $1 AND tenant_id = $2`, [learnerId, tenantId]);
    if (learnerCheck.rows.length === 0 || learnerCheck.rows[0].role !== 'learner') {
      throw { status: 400, error: 'Invalid learner ID', code: 'INVALID_LEARNER' };
    }

    const parentCheck = await query(`SELECT role FROM users WHERE id = $1 AND tenant_id = $2`, [parentId, tenantId]);
    if (parentCheck.rows.length === 0 || parentCheck.rows[0].role !== 'parent') {
      throw { status: 400, error: 'Invalid parent ID', code: 'INVALID_PARENT' };
    }

    const res = await query(
      `INSERT INTO learner_profiles (learner_id, guardian_id, consent_given, consent_given_at)
       ...
       RETURNING *`,
      [learnerId, parentId, consentGiven]
    );
   ...
   */
  static async listDevices(tenantId: string) {
    const res = await query(
      `SELECT id, name, room, tenant_id, device_token, platform, os_version, app_version, is_active, registered_at, last_seen_at
       FROM devices
       WHERE tenant_id = $1
   ...
   */
  static async registerDevice(tenantId: string, params: { name: string; room?: string; platform?: string; os_version?: string }, adminId: string) {
    // Generate secure 256-bit (32 bytes) random hex string
    const deviceToken = crypto.randomBytes(32).toString('hex');

    const res = await query(
      `INSERT INTO devices (name, room, tenant_id, device_token, platform, os_version, is_active, registered_at)
       VALUES ($1, $2, $3, $4, $5, $6, true, NOW())
       RETURNING *`,
      [params.name, params.room || null, tenantId, deviceToken, params.platform || 'windows', params.os_version || null]
    );
   ...
   */
  static async updateDevice(tenantId: string, deviceId: string, params: { name?: string; room?: string; is_active?: boolean }, adminId: string) {
    const res = await query(
      `UPDATE devices
       SET name = COALESCE($1, name),
       ...
       WHERE id = $4 AND tenant_id = $5
       RETURNING *`,
      [params.name || null, params.room || null, params.is_active !== undefined ? params.is_active : null, deviceId, tenantId]
    );
   ...
   */
  static async deactivateDevice(tenantId: string, deviceId: string, adminId: string) {
    const res = await query(
      `UPDATE devices SET is_active = false WHERE id = $1 AND tenant_id = $2 RETURNING *`,
      [deviceId, tenantId]
    );
  ...
  static async listCurricula() {
    const res = await query(`SELECT id, code, name, country, region, description, version FROM curricula ORDER BY name`);
    return res.rows;
  }
   ...
   */
  static async activateCurriculum(tenantId: string, params: { curriculum_id: string; subjects?: string[]; grade_min?: number; grade_max?: number }, adminId: string) {
    const res = await query(
      `INSERT INTO tenant_config (tenant_id, active_curriculum_id, active_subjects, grade_min, grade_max, updated_at, updated_by)
  ...
  static async getDashboardStats(tenantId: string) {
    ...
    const learnerRes = await query(
      `SELECT COUNT(*) FILTER (WHERE is_active = true) as active_learners, COUNT(*) as total_learners FROM users WHERE tenant_id = $1 AND role = 'learner'`,
      [tenantId]
    );
    ...
    const escalationRes = await query(
      `SELECT COUNT(*) FILTER (WHERE status = 'pending' OR status = 'attending') as open_escalations,
    ...
    const masteryRes = await query(
      `SELECT AVG(lcs.mastery_score) as avg_mastery
    ...
    const deviceRes = await query(
      `SELECT COUNT(*) FILTER (WHERE is_active = true) as total_devices,
   ...
   */
  static async getAtRiskLearners(tenantId: string) {
    const res = await query(
      `SELECT u.id as learner_id, u.name, u.email, lp.grade,
   ...
   */
  static async listConsentRecords(tenantId: string) {
    const res = await query(
   ...
   */
  static async createDataExportRequest(tenantId: string, learnerId: string, adminId: string) {
    const res = await query(
      `INSERT INTO data_requests (learner_id, requested_by, tenant_id, request_type, status, requested_at, notes, file_url)
       VALUES ($1, $2, $3, 'export', 'completed', NOW(), 'GDPR learner data package generated by admin', $4)
       RETURNING *`,
      [learnerId, adminId, tenantId, `/downloads/export-${learnerId}.json`]
    );
   ...
   */
  static async createDataDeletionRequest(tenantId: string, learnerId: string, adminId: string) {
    const res = await query(
      `INSERT INTO data_requests (learner_id, requested_by, tenant_id, request_type, status, requested_at, notes)
       VALUES ($1, $2, $3, 'deletion', 'pending', NOW(), 'Scheduled for data deletion by admin request')
       RETURNING *`,
      [learnerId, adminId, tenantId]
    );
   ...
   */
  static async getAuditLogs(tenantId: string, options: { userId?: string; action?: string; fromDate?: string; toDate?: string; limit?: number }) {
   ...
   */
  static async exportAuditLogsCsv(tenantId: string) {
