...
export class MentorService {
   ...
   */
  static async getLiveDashboard(mentorId: string, tenantId: string) {
    ...
    const enriched = await Promise.all(
      learners.map(async (learner: any) => {
        ...
        const sessionRes = await query(
          `SELECT s.id, s.status, s.started_at, s.total_duration_min,
        ...
        // Derive status
        let status = 'on_track';
        if (escalation?.status === 'pending') status = 'needs_help';
        else if (aiInteractions >= 4) status = 'slow_progress';
    ...
    const pendingEscRes = await query(
      `SELECT COUNT(*) as count
       FROM escalations e
       JOIN users u ON u.id = e.learner_id
       WHERE u.tenant_id = $1 AND e.status = 'pending'`,
      [tenantId]
    );
   ...
   */
  static async getActiveEscalations(tenantId: string) {
    const res = await query(
      `SELECT e.id, e.learner_id, e.trigger_type, e.brief_text,
   ...
   */
  static async getLearnerDetail(learnerId: string, tenantId: string) {
    // Learner profile
    ...
    if (userRes.rows.length === 0) throw { status: 404, error: 'Learner not found', code: 'NOT_FOUND' };
    const learner = userRes.rows[0];

    // Competency states
    const compRes = await query(
      `SELECT lcs.competency_id as id, c.topic as name,
    ...
    const sessRes = await query(
      `SELECT s.id, s.planned_at as date, s.session_goal as topic,
    ...
    const goalsRes = await query(
      `SELECT id, goal_type, description as text, target_date, status
   ...
   */
  static async getTodaySession(learnerId: string) {
    const sessionRes = await query(
      `SELECT s.id, s.planned_at as date, s.session_goal as topic, s.status
       FROM sessions s
       WHERE s.learner_id = $1
    ...
    const eventsRes = await query(
      `SELECT id, event_type as type, payload, timestamp as time
       FROM session_events
       WHERE session_id = $1
   ...
   */
  static async getNotes(learnerId: string) {
    const res = await query(
      `SELECT mn.id, mn.learner_id, mn.note_type, mn.note_text as text,
  ...
  static async addNote(params: {
    ...
    const res = await query(
      `INSERT INTO mentor_notes (learner_id, mentor_id, note_type, note_text, competency_id, is_parent_visible)
       VALUES ($1, $2, $3, $4, $5, $6)
   ...
   */
  static async attendEscalation(escalationId: string, mentorId: string) {
    const res = await query(
      `UPDATE escalations
       SET status = 'attending', assigned_mentor_id = $2, attended_at = NOW()
       WHERE id = $1
       RETURNING *`,
      [escalationId, mentorId]
    );
    if (res.rows.length === 0) throw { status: 404, error: 'Escalation not found', code: 'NOT_FOUND' };
   ...
   */
  static async resolveEscalation(escalationId: string, mentorId: string, resolutionNote?: string) {
    const res = await query(
      `UPDATE escalations
       SET status = 'resolved', resolved_at = NOW(), resolution_note = $3
       WHERE id = $1 AND assigned_mentor_id = $2
       RETURNING *`,
      [escalationId, mentorId, resolutionNote || null]
    );
    if (res.rows.length === 0) throw { status: 404, error: 'Escalation not found or not assigned to you', code: 'NOT_FOUND' };
    return { success: true, escalation: res.rows[0] };
  }
   ...
   */
  static async dismissEscalation(escalationId: string, mentorId: string) {
    const res = await query(
      `UPDATE escalations SET status = 'dismissed' WHERE id = $1 RETURNING *`,
      [escalationId]
    );
    if (res.rows.length === 0) throw { status: 404, error: 'Escalation not found', code: 'NOT_FOUND' };
    return { success: true };
   ...
   */
  static async getSummary(mentorId: string, tenantId: string) {
    const sessionsRes = await query(
      `SELECT COUNT(*) as count
       ...
       WHERE lma.mentor_id = $1
         AND u.tenant_id = $2
         AND s.planned_at >= CURRENT_DATE - INTERVAL '7 days'`,
      [mentorId, tenantId]
    );
    ...
    const avgRes = await query(
      `SELECT ROUND(AVG(EXTRACT(EPOCH FROM (s.ended_at - s.started_at)) / 60)) as avg_min
       FROM sessions s
       JOIN learner_mentor_assignments lma ON lma.learner_id = s.learner_id
       WHERE lma.mentor_id = $1
    ...
    const strugglesRes = await query(
    ...
    return {
      ...
      top_struggles: strugglesRes.rows.map((r: any) => ({ topic: r.topic, count: parseInt(r.count) })),
      at_risk_learners: [],
    };
  }
}
...
function formatEventLabel(eventType: string, payload: any): string {
