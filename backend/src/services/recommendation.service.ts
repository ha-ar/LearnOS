...
export class RecommendationService {
   ...
   */
  static async runEvidenceEngine(sessionId: string, learnerId: string) {
    // 1. Fetch check question events for this session
    const eventsRes = await query(
      `SELECT competency_id, payload, event_type
       FROM session_events
       WHERE session_id = $1 AND competency_id IS NOT NULL
         AND event_type IN ('check_question_answered', 'check_question_correct', 'check_question_incorrect')`,
      [sessionId]
    );
    ...
    for (const [compId, stats] of grouped.entries()) {
      ...
      // total >= 1 => emerging
      let level = 'emerging';
      if (score >= 0.85 && stats.total >= 5) level = 'mastered';
      else if (score >= 0.70 && stats.total >= 3) level = 'proficient';
      else if (score >= 0.50 && stats.total >= 2) level = 'developing';
      ...
      await query(
        `INSERT INTO learner_competency_states (
         ...
         SET mastery_level = EXCLUDED.mastery_level,
             ...
             updated_at = NOW()`,
        [learnerId, compId, level, score, stats.total, stats.correct, summary]
      );
    ...
    // Log evidence run
    await query(
      `INSERT INTO evidence_runs (session_id, learner_id, run_type, events_processed, competencies_updated)
       VALUES ($1, $2, 'full', $3, $4)`,
      [sessionId, learnerId, events.length, updatedCount]
    );
   ...
   */
  static async getNextBestAction(input: NextActionInput) {
    ...
    await query(
      `INSERT INTO recommendation_log (session_id, learner_id, action_type, rule_fired, confidence, evidence)
       VALUES ($1, $2, $3, $4, $5, $6)`,
      [
        input.session_id,
        input.learner_id,
