...
import { JwtPayload, UserRecord, UserRole } from '../types/auth.js';
...
export class AuthService {
  ...
  static async logAudit(params: {
    ...
    try {
      await query(
        `INSERT INTO audit_log (actor_id, actor_role, tenant_id, action, resource_type, resource_id, result, reason)
  ...
  static async login(email: string, password: string, deviceId?: string) {
    ...
    const userRes = await query<UserRecord>(
      `SELECT id, email, password_hash, name, role, tenant_id, is_active, created_at
  ...
  static async refreshToken(rawRefreshToken: string) {
    ...
    const tokenRes = await query(
      `SELECT r.id, r.user_id, r.expires_at, r.revoked_at, u.email, u.name, u.role, u.tenant_id, u.is_active
   ...
   */
  static async logout(rawRefreshToken: string, actorId: string, tenantId: string) {
    if (rawRefreshToken) {
      const tokenHash = this.hashToken(rawRefreshToken);
  ...
  static async registerDevice(params: {
    ...
    const rawDeviceToken = `dev_${crypto.randomBytes(32).toString('hex')}`;

    const res = await query(
      `INSERT INTO devices (name, room, tenant_id, device_token, platform)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING id, name, room, tenant_id, device_token, platform, registered_at`,
      [params.name, params.room || null, params.tenant_id, rawDeviceToken, params.platform || 'macos']
    );
  ...
  static async createUser(params: {
    ...
    const userRes = await query(
      ...
      [params.email, passwordHash, params.name, params.role, params.tenant_id]
    );
  ...
  static async recordConsent(params: {
    ...
    const res = await query(
      `INSERT INTO consent_records (learner_id, guardian_id, consent_version, data_scope, consent_method, consented_at, admin_who_recorded)
       VALUES ($1, $2, $3, $4, $5, NOW(), $6)
       ON CONFLICT (learner_id, consent_version) DO UPDATE
         SET revoked_at = NULL, consented_at = NOW()
       RETURNING id, learner_id, guardian_id, consent_version, data_scope, consented_at`,
      [params.learner_id, params.guardian_id, version, scope, method, params.admin_id || null]
    );

    // Update learner profile consent flag
    await query(
      `UPDATE learner_profiles SET consent_given = true, consent_given_at = NOW() WHERE learner_id = $1`,
      [params.learner_id]
    );
