...
const fallbackEventsList = [...initialMockEvents];
...
function handleFallbackQuery(text, params = []) {
    ...
    if (normalized.includes('insert into users')) {
        ...
        return { rows: [newUser], command: 'INSERT', rowCount: 1, oid: 0, fields: [] };
    }
    // 4. Weekly Reports
    if (normalized.includes('from weekly_reports')) {
        ...
        return { rows: filtered, command: 'SELECT', rowCount: filtered.length, oid: 0, fields: [] };
    }
    if (normalized.includes('insert into weekly_reports') || normalized.includes('update weekly_reports')) {
        ...
        if (normalized.includes('set status = \'sent\'')) {
            ...
            if (idx >= 0)
                fallbackReportsList[idx] = report;
            else
                fallbackReportsList.unshift(report);
        }
        return { rows: [report], command: 'MUTATION', rowCount: 1, oid: 0, fields: [] };
    }
    ...
    if (normalized.includes('insert into session_events')) {
        ...
        return { rows: [newEvt], command: 'INSERT', rowCount: 1, oid: 0, fields: [] };
    }
    ...
    if (normalized.includes('from resources')) {
        ...
        if (normalized.includes('count(')) {
            return { rows: [{ count: seedResources.length }], command: 'SELECT', rowCount: 1, oid: 0, fields: [] };
        }
        return { rows: seedResources, command: 'SELECT', rowCount: seedResources.length, oid: 0, fields: [] };
    }
