...
class TwinService {
     ...
     */
    static async getTwinSummary(learnerId) {
        // 1. Fetch competencies states
        const compRes = await (0, index_js_1.query)(`SELECT lcs.id, lcs.competency_id, c.subject, c.topic, c.subtopic, c.grade_level,
       ...
       ORDER BY lcs.updated_at DESC`, [learnerId]);
        ...
        const profileRes = await (0, index_js_1.query)(`SELECT preferred_format, preferred_session_length_min, hint_response, pace_signal, notes
       FROM learner_profiles_twin WHERE learner_id = $1`, [learnerId]);
        // 3. Fetch active goals
        const goalsRes = await (0, index_js_1.query)(`SELECT id, goal_type, description, target_date, status, created_at
       FROM learner_goals WHERE learner_id = $1 AND status = 'active'
       ORDER BY created_at DESC`, [learnerId]);
        // 4. Fetch recent portfolio items
        const portfolioRes = await (0, index_js_1.query)(`SELECT id, item_type, title, description, file_url, competency_ids, created_at
       FROM portfolio_items WHERE learner_id = $1
       ORDER BY created_at DESC LIMIT 5`, [learnerId]);
        // 5. Fetch recent mentor notes
        const notesRes = await (0, index_js_1.query)(`SELECT id, mentor_id, note_type, note_text, is_parent_visible, created_at
       FROM mentor_notes WHERE learner_id = $1
       ORDER BY created_at DESC LIMIT 5`, [learnerId]);
        // 6. Fetch recent session events
        const eventsRes = await (0, index_js_1.query)(`SELECT id, session_id, event_type, competency_id, resource_id, payload, timestamp
       FROM session_events WHERE learner_id = $1
       ORDER BY timestamp DESC LIMIT 10`, [learnerId]);
        return {
     ...
     */
    static async getCompetencies(learnerId) {
        const res = await (0, index_js_1.query)(`SELECT lcs.id, lcs.competency_id, c.subject, c.topic, c.subtopic, c.grade_level,
       ...
       JOIN competencies c ON c.id = lcs.competency_id
       WHERE lcs.learner_id = $1
       ORDER BY c.subject, c.topic`, [learnerId]);
        return res.rows;
    }
    /**
     * Get specific competency state for a learner
     */
    static async getCompetencyById(learnerId, competencyId) {
        const res = await (0, index_js_1.query)(`SELECT lcs.id, lcs.competency_id, c.subject, c.topic, c.subtopic, c.grade_level,
       ...
       FROM learner_competency_states lcs
       JOIN competencies c ON c.id = lcs.competency_id
       WHERE lcs.learner_id = $1 AND lcs.competency_id = $2`, [learnerId, competencyId]);
        if (res.rows.length === 0) {
            // Check if competency exists in global catalogue
            const catRes = await (0, index_js_1.query)(`SELECT * FROM competencies WHERE id = $1`, [competencyId]);
     ...
     */
    static calculateMasteryLevel(scoreRatio, attemptsTotal) {
        if (attemptsTotal === 0)
            return 'not_started';
        if (attemptsTotal < 3 && scoreRatio < 0.5)
            return 'emerging';
        if (scoreRatio >= 0.85)
            return 'mastered';
        if (scoreRatio >= 0.65)
    ...
    static async updateCompetencyMastery(params) {
        ...
        const existingRes = await (0, index_js_1.query)(`SELECT attempts_total, attempts_correct, learner_confidence, evidence_summary
       FROM learner_competency_states
       WHERE learner_id = $1 AND competency_id = $2`, [params.learner_id, params.competency_id]);
        ...
        const compRes = await (0, index_js_1.query)(`SELECT topic FROM competencies WHERE id = $1`, [params.competency_id]);
        ...
        const res = await (0, index_js_1.query)(`INSERT INTO learner_competency_states (
     ...
     */
    static async getSessionEvents(learnerId, options) {
    ...
    static async emitSessionEvent(params) {
        return event_service_js_1.EventService.ingestEvent({
     ...
     * Get learner goals
     */
    static async getGoals(learnerId) {
        const res = await (0, index_js_1.query)(`SELECT * FROM learner_goals WHERE learner_id = $1 ORDER BY created_at DESC`, [learnerId]);
        return res.rows;
    }
    /**
    ...
    static async createGoal(params) {
        const res = await (0, index_js_1.query)(`INSERT INTO learner_goals (learner_id, goal_type, description, target_date, status)
     ...
     */
    static async getPortfolio(learnerId) {
        const res = await (0, index_js_1.query)(`SELECT * FROM portfolio_items WHERE learner_id = $1 ORDER BY created_at DESC`, [learnerId]);
        return res.rows;
    }
    /**
    ...
    static async createPortfolioItem(params) {
        const res = await (0, index_js_1.query)(`INSERT INTO portfolio_items (learner_id, item_type, title, description, file_url, competency_ids)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING *`, [
            params.learner_id,
     ...
     * Get mentor notes
     */
    static async getMentorNotes(learnerId) {
        const res = await (0, index_js_1.query)(`SELECT mn.*, u.name as mentor_name
    ...
    static async createMentorNote(params) {
        const res = await (0, index_js_1.query)(`INSERT INTO mentor_notes (learner_id, mentor_id, tenant_id, session_id, competency_id, note_type, note_text, is_parent_visible)
     ...
     */
    static async updateTwinProfile(learnerId, params) {
        const res = await (0, index_js_1.query)(`INSERT INTO learner_profiles_twin (
