...
const safety_service_js_1 = require("./safety.service.js");
class AiService {
    static getAiClient() {
     ...
     */
    static async generateLearnYourWayExplanation(params) {
        const { concept, topic = 'Fractions & Ratios', learner_name = 'Learner', grade_level = 'Grade 6', mastery_level = 'Intermediate', explanation_style = 'real_world', pacing_speed = 'Balanced Pace', } = params;
...
Respond strictly in JSON with these keys:
{
  ...
  "quick_check_question": "One quick single-line understanding check question for the student"
     ...
     */
    static async generateTutoringResponse(params) {
        ...
        const safetyCheck = safety_service_js_1.SafetyService.checkContentSafety(prompt);
        if (!safetyCheck.safe) {
...
5. Always celebrate effort and curiosity! Never give letter grades or marks.`;
            ...
            const postSafety = safety_service_js_1.SafetyService.checkContentSafety(replyText);
...
4. Output valid JSON:
{
  ...
  "suggestion": "optional tip"
     ...
     */
    static async generateSmartHint(questionText, hintLevel = 1) {
        const ai = this.getAiClient();
        if (!ai) {
            if (hintLevel === 1)
                return 'Think about what the numerator (top number) and denominator (bottom number) represent!';
            if (hintLevel === 2)
                return 'Try multiplying both top and bottom by the exact same number to find an equivalent fraction.';
...
Write a warm, 3-sentence encouraging summary for the parent's weekly report. Highlight their effort and key accomplishment in plain, positive language.`,
     ...
     */
    static getFallbackTutoringReply(prompt, topic, learnerName) {
     ...
     */
    static async generateLessonContent(params) {
        const { topic, competency_description = '', grade_level = 'Grade 6', learner_name = 'Learner', mastery_level = 'emerging', subject = 'Mathematics', } = params;
        const ai = this.getAiClient();
