...
class ResourceService {
    ...
    `;
        ...
        const res = await (0, index_js_1.query)(sql, queryParams);
        return res.rows;
    }
    /**
     * Get resource by ID
    ...
    static async getResourceById(id) {
        const res = await (0, index_js_1.query)(`SELECT r.id, r.provider_id, rp.name as provider_name, r.external_id, r.title,
              r.description, r.format, r.url, r.embed_url, r.duration_min,
              r.grade_min, r.grade_max, r.language, r.is_age_safe, r.attribution,
    ...
    `;
        const queryParams = [params.competency_id];
        if (excludeIds.length > 0) {
            sql += ` AND r.id NOT IN (${excludeIds.map((_, i) => `$${i + 2}`).join(',')})`;
            queryParams.push(...excludeIds);
        }
        const candidatesRes = await (0, index_js_1.query)(sql, queryParams);
     ...
     */
    static async getQuizQuestions(resourceId, includeAnswers = false) {
        const sql = includeAnswers
         ...
         FROM quiz_questions WHERE resource_id = $1 ORDER BY display_order ASC`;
        const res = await (0, index_js_1.query)(sql, [resourceId]);
        return res.rows;
    }
    /**
    ...
    static async listProviders() {
        const res = await (0, index_js_1.query)(`SELECT * FROM resource_providers WHERE is_active = true ORDER BY name ASC`);
        return res.rows;
    }
}
