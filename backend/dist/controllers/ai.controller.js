...
class AiController {
     ...
     * POST /api/ai/tutor
     */
    static async tutorInteraction(req, res) {
        try {
            ...
            const result = await ai_service_js_1.AiService.generateTutoringResponse({
                prompt,
                learner_name,
                topic,
                history,
    ...
    static async getHint(req, res) {
        try {
            ...
            const hint = await ai_service_js_1.AiService.generateSmartHint(question_text, hint_level || 1);
    ...
    static async explainConcept(req, res) {
        try {
            ...
            const result = await ai_service_js_1.AiService.generateLearnYourWayExplanation({
                concept,
                topic,
                learner_name,
                mastery_level,
     ...
     */
    static async generateLesson(req, res) {
        try {
            ...
            if (competency_id) {
                const compRes = await (0, index_js_1.query)(`SELECT topic, description, grade_level FROM competencies WHERE id = $1`, [competency_id]);
            ...
            const lesson = await ai_service_js_1.AiService.generateLessonContent({
                topic,
